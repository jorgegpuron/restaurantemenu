...
if (!existsSync(cliente('carta.json'))
    && existsSync(cliente('menu.md'))) {
  abortar(
    "Hay un menu.md pero no hay carta.json: ese menu es del restaurante anterior.",
    "escribe la carta del restaurante en carta.json y ejecuta: node importar.mjs");
}
...
const carpetaCliente = CARPETA_CLIENTE;
...
const renderItem = (it, showSlot, icon, catName) => {
  ...
  /* «Para llevar»: pastilla de TEXTO en la línea de etiquetas, delante de vegano y sin gluten.
     El icono redondo de al lado de la cámara se probó en producción y no convenció (15 sep
     2026, decisión del propietario): un dibujo obliga a adivinar, y esto tiene nombre.

     VACÍA, y el texto lo pone el runtime con tr('Takeaway') — exactamente como las dos ranuras
     de al lado, oferta y destacado, y por el mismo motivo: lo que va dentro depende del estado
     del día, no del plato. Horneando aquí la palabra con sus data-<idioma> (que es como van las
     pastillas de dieta, porque aquéllas sí son del plato) el HTML engordaba 26 KB en crudo:
     312 filas repitiendo la misma palabra tres veces, una por idioma. La clave 'Takeaway' ya
     existía y ya estaba traducida: la usa el filtro de la cabecera.

     Ranura PROPIA, y no se puede reutilizar la del destacado: la fila emite UN solo
     `.item-tag-high`, así que pintar esto ahí borraría «Más vendido». Se probó y pasó
     exactamente eso. Sale oculta: sin JavaScript y sin estado la carta sigue siendo correcta,
     sólo que sin las novedades del día.

     DELANTE de dietMarks: lo pidió así el propietario, y además se lee en ese orden — primero
     lo que puedes hacer con el plato, después de qué está hecho. */
...
/* «Para llevar» en la ficha: el mismo aire de 6 contra el nombre que las dietas, y sin el
   margin-right de 8 que .item-tag lleva para separarse del nombre EN LA FILA — aquí lo que
   tiene detrás son las dietas, que ya ponen su propio 6. Con los dos, el hueco salía a 14 y
   las tres pastillas dejaban de leerse como un grupo.
   El text-shadow del h2 lo heredaría el texto del badge y sobre el negro sólo lo ensucia: la
   sombra está para despegar el nombre de la FOTO, y una pastilla rellena ya se despega sola. */
.dsheet-nombre .item-tag-llevar{margin-left:6px;margin-right:0;text-shadow:none}
.dsheet-precio{
...
/* Regla unica de separacion entre badges: 4px siempre que uno vaya pegado a otro --
   ...
   Con ~ y no con +: la fila emite SIEMPRE las tres ranuras (oferta, destacado y «para
   llevar») y las que no van se quedan con hidden, asi que el hermano inmediato de un badge
   visible puede ser una ranura vacia. Con +
   ...
   badge visible».
   OJO con la letra pequena, medida el 15 sep 2026: en la practica da 4 a TODOS, tambien al
   ultimo. Detras de todos va .sold-out-flag, que la esconde el CSS con display:none y NO con
   el atributo hidden, asi que el «~ :not([hidden])» la encuentra siempre. Los 8 hacia el
   nombre que este comentario prometia no llegan a ocurrir con el marcado de hoy. Se deja
   dicho en vez de «arreglarlo»: 4 es lo que se ve y lo que el propietario ha aprobado mirando
   la carta, y cambiarlo ahora movería todas las lineas de etiquetas de la carta. CAR-35 mide
   que el badge de «para llevar» reciba lo MISMO que sus vecinas, sea cual sea ese numero.
   Esta regla es tambien la que da su hueco a «para llevar» por los dos lados, sin necesitar
   ninguna escrita a su nombre: es un .item-tag mas dentro de .item-tags. */
.item-tags > .item-tag:not([hidden]):has(~ :not([hidden])){margin-right:4px}
/* ---- «Para llevar»: pastilla RELLENA, la unica de esta linea que va en negro ----
   Fondo --solid y texto --solid-ink, que son literalmente «el relleno de un chip solido y su
   texto» y el unico par de la paleta con 4,5:1 GARANTIZADO: lo comprueba verificarPaleta() en
   cada build. Ademas son constantes del motor —no se derivan de colorPrincipal—, asi que este
   badge se ve exactamente igual en todos los clientes, que es lo que se quiere de una etiqueta
   que dice un hecho del servicio y no un rasgo de la marca.
   Por que negro y no el acento: el acento ya lo llevan la oferta (relleno) y las dietas
   (contorno). Un tercer naranja en la misma linea convierte tres cosas distintas en una mancha;
   el negro las separa sin inventar un color que no este declarado.
   Hereda el resto de .item-tag —12px, versalitas, 16 de line-height, caja de 18— asi que no
   descuadra la linea ni cambia --tags-line. */
.item-tag-llevar{background:var(--solid);color:var(--solid-ink)}
...
@media (max-width:767px){
  ...
  /* Alergenos y camara van en la linea del NOMBRE (los badges, en la de arriba): centrados
     ...
     «Para llevar» ya no esta aqui: es una pastilla de texto y vive arriba con las demas. */
  ...
  /* Fila CON camara (.abre): el circulo de 32 (30 con sus margenes) estira la linea del nombre,
     ...
     Ya NO hay que contemplar «para llevar» aqui: desde que es una pastilla de texto no vive en
     la linea del nombre y no la estira. Lo que la destapa en movil es .has-tags, como a
     cualquier otro badge — ver el toggle junto a render(). */
...
(function () {
  ...
  function marcasAlergeno(fila, claves) {
    ...
    /* SIEMPRE delante de la cámara, nunca detrás. La cámara (.has-photo) la añade render() al
       final del h3 una sola vez; esto se llama en cada pasada que aplica los alérgenos
  ...
  function render() {
    ...
    document.querySelectorAll('.single-menu-items[data-key]').forEach(function (row) {
      ...
      /* «Para llevar», en su ranura de .item-tags y sin tocar la del destacado. Con caída a la
         clave vieja, como todo lo demás: un estado escrito por el panel anterior a la migración
         se sigue aplicando. La fila se marca además con data-llevar para que el buscador filtre
         por esto sin volver a leer el estado.
         El TEXTO se repone en CADA pasada, no sólo al encenderlo, igual que el del destacado
         de aquí arriba: render() vuelve a correr al cambiar de idioma, y sin esto la pastilla
         se quedaría diciendo «Para llevar» en el idioma con el que se cargó la página. */
      var bolsa = row.querySelector('.item-tag-llevar');
      var esLlevar = llevar.indexOf(key) !== -1 || (leg && llevar.indexOf(leg) !== -1);
      if (bolsa) {
        bolsa.textContent = esLlevar ? tr('Takeaway') : '';
      ...
      /* El número ya no vive aquí, así que la línea de etiquetas —y el desplazamiento del
         ...
         «PARA LLEVAR» CUENTA IGUAL, y no es opcional. Esa misma línea es la que el móvil
         esconde con display:none, y un plato marcado SOLO para llevar no enciende ninguna de
         las otras tres: sin contarlo aquí, su badge se queda dentro de una línea oculta y se
         ve en tablet y escritorio pero no en el teléfono. Es exactamente el defecto que se
         arregló el 15 sep 2026 sacándolo de aquí, y que vuelve al devolverlo. CAR-39 lo mide. */
      var llevarBadge = row.querySelector('.item-tag-llevar');
      row.classList.toggle('has-tags',
        (alto && !alto.hidden) || (oferta && !oferta.hidden)
        || (llevarBadge && !llevarBadge.hidden) || !!row.querySelector('.diet-marks'));
    });
  ...
  document.addEventListener('${CLIENTE.slug}:lang', pintarWa);
  pintarWa();
...
` : ''}
  ...
  if (dsQ) {
    ...
    document.addEventListener('${CLIENTE.slug}:lang', function () { dsPlaceholder(); dsCuentas(); dsPintar(); });
    dsPlaceholder();
    dsCuentas();
  }
  ...
  function rellenarFicha(carta, row) {
    ...
    /* «Para llevar» ANTES que vegano y sin gluten, el mismo orden que en la fila: quien abre la
       ficha ya vio la fila, y dos órdenes distintos para las mismas etiquetas se leen como dos
       cosas distintas. Va después del textContent de arriba, que ya vació lo que hubiera dejado
       la diapositiva anterior — la ficha reutiliza las mismas tarjetas al pasar de plato.
       Sólo si la fila lo tiene VISIBLE: la ranura existe en los 312 platos y clonarla sin mirar
       metería un badge oculto en cada ficha. Se clona de la fila y no se compone aquí, así que
       llega con el texto que render() acaba de poner en el idioma de ahora: al cambiar de
       idioma con la ficha abierta, rellenarFicha() se vuelve a llamar y trae el nuevo. */
    var llevar = h3 ? h3.querySelector('.item-tag-llevar') : null;
    if (llevar && !llevar.hidden) fichaNombre.appendChild(llevar.cloneNode(true));
...
{
  const htaccess = readFileSync(cliente('server/.htaccess'), 'utf8');
...
const copiar = (desdeUrl, hasta) => {
  ...
  copyFileSync(desdeUrl, hasta);
};
const enMotor = (f) => new URL(f, new URL('./', import.meta.url));

/* Los ficheros sueltos, con su ORIGEN explicito: los del cliente salen de su raiz, los del
...
/* Carpetas enteras, solo el primer nivel. Las subcarpetas que hay al otro lado las crea el
   panel en el servidor y no tienen original aqui: assets/hero/, assets/platos/ y
   admin/copias/. Subir cualquiera encima de la del hosting borraria trabajo del cliente.

   admin/ se ensambla de DOS origenes y el orden importa: primero el codigo del panel, que es
   del motor; despues lo del cliente (su .htaccess con la ruta publica y lo generado por este
   build), que PISA a lo del motor si un nombre coincide. El cliente siempre gana. */
const CARPETAS = [
  ...
  [enMotor('assets/banderas/'), 'assets/banderas/'],
];
...
for (const [desde, destino] of CARPETAS) {
  for (const e of readdirSync(desde, { withFileTypes: true })) {
    if (!e.isFile()) continue;                     // assets/hero/ y compania
    if (NO_SUBIR.has(e.name)) { dejados.push(e.name); continue; }
    const destinoUrl = new URL(destino + e.name, SUBIR);
